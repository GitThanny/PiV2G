...
class CanNiuera:
    ...
    def __init__(self, interface="socketcan", channel="can2", bitrate=125000):
        if can is None:
    ...
    def _send_command(self, data, dst_addr=None):
        ...
        while self._heartbeat_active:
            try:
                if self.started and keep_alive_read:
                    self._send_command_read_reg(self.REG_READ_DC_VI)
            except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # NOTE: If your EV limits block the test, set these high enough
        charger.setEvseMaxCurrent(50)

        charger.start()
        time.sleep(2.0) # Wait for relays to close

        hold_s = 5.0
        read_interval_s = 0.5

        # Forward then reverse sequence matching your request
        forward = [(200, 2), (300, 3), (400, 4), (500, 5)]
        reverse = [(500, 5), (400, 4), (300, 3), (200, 2)]
        sequence = forward + reverse

        print("[TEST] Sequence: Up to 500V/5A, then down to 200V/2A (5s hold each)")

        for target_v, target_i in sequence:
            # 1. Command Voltage and Current
            ok_v = charger.setEvTargetVoltage(float(target_v))
            ok_i = charger.setEvTargetCurrent(float(target_i))
            
            if not ok_v or not ok_i:
                print(f"[ERROR] setEvTarget rejected by limit check.")
                break

            print(f"\n=== SET TARGET {target_v:.1f}V | {target_i:.1f}A (hold {hold_s:.1f}s) ===")
            
            # 2. Loop for 5 seconds to measure and print
            t0 = time.time()
            while (time.time() - t0) < hold_s:
                meas_v = charger.getEvsePresentVoltage()
                meas_i = charger.getEvsePresentCurrent()
                
                print(f"       Target={target_v:6.1f}V {target_i:4.1f}A  |  Measured={meas_v:6.1f}V {meas_i:5.2f}A")
