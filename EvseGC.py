...
class Evse():
    def __init__(self, iftype, iface, mac, auto_authorize=False):
        ...
        self.relay.turn_on()
        self.CanNiuera = CanNiuera()
        self.CanNiuera.StartCanLoop()
        self.schedule = None
    ...
    def _handleNetworkEstablished(self):
        ...
        self.dc_charging_parameters = {
            'isolation_level': 1,
            'min_voltage': self.CanNiuera.getEvseMinVoltage(),
            'min_current': self.CanNiuera.getEvseMinCurrent(),
            'max_voltage': self.CanNiuera.getEvseMaxVoltage(),
            'max_current': self.CanNiuera.getEvseMaxCurrent(),
            'max_power': self.CanNiuera.getEvseMaxPower(),
            'peak_current_ripple': int(self.CanNiuera.getEvseDeltaCurrent()),
            'status': 0
        }
        ...
        self.ac_charging_parameters = {
            'rcd_status': 0,
            'nominal_voltage': self.CanNiuera.getEvseMaxVoltage(),
            'max_current': self.CanNiuera.getEvseMaxCurrent(),
        }
        ...
        while True:
            if self.charging:
                ...
                charging_parameters = {
                    'isolation_level': 1,
                    'present_voltage': int(self.CanNiuera.getEvsePresentVoltage()),
                    'present_current': int(self.CanNiuera.getEvsePresentCurrent()),
                    'max_voltage': int(self.CanNiuera.getEvseMaxVoltage()),
                    'max_current': int(self.CanNiuera.getEvseMaxCurrent()),
                    'max_power': int(self.CanNiuera.getEvseMaxPower()),
                    'status': 0,
                }
                
                # ===== DEBUG PRINT ADDED HERE =====
                #print(f"[DEBUG] Charging Loop - V: {charging_parameters['present_voltage']}V, I: {charging_parameters['present_current']}A, P: {charging_parameters['max_power']}W")
                # ==================================
    ...
    def _handleEnergyTransferModeSelected(self, data):
        ...
        self.CanNiuera.setEvMaxVoltage(message['max_voltage'])

        if 'min_current' in message:
            print('Minimum current: {}'.format(message['min_current']))
            self.CanNiuera.setEvMinCurrent(message['min_current'])

        print('Maximum current: {}'.format(message['max_current']))
        self.CanNiuera.setEvMaxCurrent(message['max_current'])

        if 'max_power' in message:
            print('Maximum power: {}'.format(message['max_power']))
            self.CanNiuera.setEvMaxPower(message['max_power'])
    ...
    def _handleDCChargeParametersChanged(self, data):
        ...
        if 'target_voltage' in message:
            ...
            self.CanNiuera.setEvTargetVoltage(message['target_voltage'])

        if 'target_current' in message:
            print("EV target current: {}A".format(message['target_current']))
            self.CanNiuera.setEvTargetCurrent(message['target_current'])
    ...
    def _handleRequestStartCharging(self, data):
        ...
        # Start fast polling loop ONLY when actual charging starts
        self.charging = True
    ...
    def getCanNiuera(self):
        """
        Returns the CanNiuera object
        """
        if hasattr(self, "CanNiuera"):
            return self.CanNiuera
        else:
            return None
